...
<div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-2xl">
        ...
        <form action="{{ route('user.biodata.update', $biodataSiswa->id) }}" method="POST" enctype="multipart/form-data" class="mb-0 mt-6 space-y-4 rounded-lg p-4 shadow-lg sm:p-6 lg:p-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    <input
                        ...
                        id="nisn"
                        name="nisn"
                        value="{{ old('nisn', $biodataSiswa->nisn ?? '') }}"
                        required
                    />
            ...
            <div>
                ...
                <div class="relative">
                    <input
                        ...
                        id="nama_lengkap"
                        name="nama_lengkap"
                        value="{{ old('nama_lengkap', $biodataSiswa->nama_lengkap ?? '') }}"
                        required
                    />
                    <span class="absolute inset-y-0 end-0 grid place-content-center px-4">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4 text-gray-400">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        id="tempat_lahir"
                        name="tempat_lahir"
                        value="{{ old('tempat_lahir', $biodataSiswa->tempat_lahir ?? '') }}"
                        required
                    />
                    @error('tempat_lahir')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="tanggal_lahir"
                        name="tanggal_lahir"
                        value="{{ old('tanggal_lahir', $biodataSiswa->tanggal_lahir ?? '') }}"
                        required
                    />
                    @error('tanggal_lahir')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <select
                    id="jenis_kelamin"
                    name="jenis_kelamin"
                    class="mt-1 block w-full rounded-lg border-gray-200 p-4 text-sm shadow-sm"
                    required
                >
                    <option value="">Pilih Jenis Kelamin</option>
                    <option value="Laki-laki" {{ (old('jenis_kelamin', $biodataSiswa->jenis_kelamin ?? '') == 'Laki-laki') ? 'selected' : '' }}>Laki-laki</option>
                    <option value="Perempuan" {{ (old('jenis_kelamin', $biodataSiswa->jenis_kelamin ?? '') == 'Perempuan') ? 'selected' : '' }}>Perempuan</option>
                </select>
                @error('jenis_kelamin')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input
                    ...
                    id="agama"
                    name="agama"
                    value="{{ old('agama', $biodataSiswa->agama ?? '') }}"
                    required
                />
                @error('agama')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea
                    ...
                    name="alamat"
                    required
                >{{ old('alamat', $biodataSiswa->alamat ?? '') }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input
                    ...
                    id="asal_sekolah"
                    name="asal_sekolah"
                    value="{{ old('asal_sekolah', $biodataSiswa->asal_sekolah ?? '') }}"
                    required
                />
                @error('asal_sekolah')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                    <label for="foto_nilai_rapor" class="block text-sm font-medium text-gray-700">Foto Nilai Rapor</label>
                    <input
                        type="file"
                        ...
                        id="foto_nilai_rapor"
                        name="foto_nilai_rapor"
                    />
                    @error('foto_nilai_rapor')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    @if(isset($biodataSiswa->foto_nilai_rapor) && $biodataSiswa->foto_nilai_rapor)
                        <p class="mt-2 text-xs text-gray-500">File saat ini: <a href="{{ asset('storage/' . $biodataSiswa->foto_nilai_rapor) }}" target="_blank" class="text-blue-600 hover:underline">Lihat</a></p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    @if(isset($biodataSiswa->foto_ijazah) && $biodataSiswa->foto_ijazah)
                        <p class="mt-2 text-xs text-gray-500">File saat ini: <a href="{{ asset('storage/' . $biodataSiswa->foto_ijazah) }}" target="_blank" class="text-blue-600 hover:underline">Lihat</a></p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    @if(isset($biodataSiswa->foto_formal) && $biodataSiswa->foto_formal)
                        <p class="mt-2 text-xs text-gray-500">File saat ini: <a href="{{ asset('storage/' . $biodataSiswa->foto_formal) }}" target="_blank" class="text-blue-600 hover:underline">Lihat</a></p>
                    @endif
                </div>
            </div>
