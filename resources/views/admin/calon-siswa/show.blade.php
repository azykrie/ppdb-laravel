...
<div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between mb-6">
        <a href="{{ route('admin.calon-siswa.index') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
            </svg>
            <span class="text-sm font-medium">Kembali ke Daftar</span>
        </a>
        <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Detail Biodata: {{ $biodataSiswa->nama_lengkap ?? 'N/A' }}</h1>
    </div>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-8">
        ...
        <div class="mb-6">
            ...
            @endphp
            <span class="inline-flex items-center justify-center rounded-full {{ $statusClass }} px-3 py-1.5 text-sm font-medium">
                {{ $statusText }}
            </span>
            ...
            @if ($biodataSiswa->verification_notes)
                <div class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
                    ...
                    @if ($biodataSiswa->verifiedBy)
                        <p class="mt-2 text-xs text-red-500">Diverifikasi oleh: {{ $biodataSiswa->verifiedBy->name }} pada {{ \Carbon\Carbon::parse($biodataSiswa->verified_at)->format('d F Y H:i') }}</p>
                    @endif
                </div>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                @if ($biodataSiswa->foto_nilai_rapor)
                    <a href="{{ asset('storage/' . $biodataSiswa->foto_nilai_rapor) }}" target="_blank" class="inline-flex items-center gap-1.5 text-blue-600 hover:underline">
                        Lihat File
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                @if ($biodataSiswa->foto_ijazah)
                    <a href="{{ asset('storage/' . $biodataSiswa->foto_ijazah) }}" target="_blank" class="inline-flex items-center gap-1.5 text-blue-600 hover:underline">
                        Lihat File
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                        </svg>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                @if ($biodataSiswa->foto_formal)
                    <a href="{{ asset('storage/' . $biodataSiswa->foto_formal) }}" target="_blank" class="inline-flex items-center gap-1.5 text-blue-600 hover:underline">
                        Lihat File
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-3">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                        </svg>
        ...
        <form action="{{ route('admin.calon-siswa.update', $biodataSiswa->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select
                    ...
                    class="mt-1 block w-full rounded-lg border-gray-200 p-3 text-sm shadow-sm"
                    required
                >
                    <option value="pending" {{ old('verification_status', $biodataSiswa->verification_status) == 'pending' ? 'selected' : '' }}>Menunggu Verifikasi</option>
                    <option value="verified" {{ old('verification_status', $biodataSiswa->verification_status) == 'verified' ? 'selected' : '' }}>Terverifikasi</option>
                    <option value="rejected" {{ old('verification_status', $biodataSiswa->verification_status) == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                </select>
            ...
            <div>
                <label for="verification_notes" class="block text-sm font-medium text-gray-700">Catatan Verifikasi (opsional)</label>
                <textarea
                    class="mt-1 w-full rounded-lg border-gray-200 p-3 text-sm shadow-sm"
                    placeholder="Contoh: Ijazah tidak jelas, Mohon upload ulang foto formal"
                    rows="4"
                    id="verification_notes"
                    name="verification_notes"
                >{{ old('verification_notes', $biodataSiswa->verification_notes) }}</textarea>
